...
public class signuphelp extends JDialog {
	...
	public signuphelp() {
		setResizable(false);
		setBounds(100, 100, 380, 657);
		getContentPane().setLayout(new BorderLayout());
		contentPanel.setBorder(new EmptyBorder(5, 5, 5, 5));
		getContentPane().add(contentPanel, BorderLayout.CENTER);
		contentPanel.setLayout(new MigLayout("", "[348px]", "[564px]"));
		
		JTextArea txtrSigningUp = new JTextArea();
		txtrSigningUp.setFont(new Font("Century Gothic", Font.PLAIN, 13));
		txtrSigningUp.setWrapStyleWord(true);
		txtrSigningUp.setEditable(false);
		...
		contentPanel.add(txtrSigningUp, "cell 0 0,alignx center,growy");
		{
